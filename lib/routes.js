...
module.exports = function (app) {
					...
					if(result.rows[0]){
						...
						client.query('SELECT * FROM team_users WHERE user_id=$1', [req.user[0].id], function(err, result) {
							if(result.rows[0]){
								...
								var team_id = uuidv4();
								client.query('INSERT INTO team (id, owner_id, coach_email, name, offer) VALUES ($1, $2, $3, $4, $5)', [team_id, req.user[0].email, req.body.name, req.body.offer], function(err, result) {
									if(err){console.log(err);}
									else {
										client.query('COMMIT')
									
										req.user[0].team_id = team_id;
									}
								});
					...
					if(result.rows[0]){
						...
						client.query('SELECT * FROM team_users WHERE user_id=$1', [req.user[0].id], function(err, result) {
							if(result.rows[0]){
								...
								var team_id = uuidv4();
								client.query('INSERT INTO team (id, coach_email, name, offer) VALUES ($1, $2, $3, $4)', [team_id, req.user[0].email, req.query.team, req.query.offer], function(err, result) {
									if(err){console.log(err);}
									else {
										client.query('COMMIT')
								...
								client.query('INSERT INTO team_users (team_id, user_id) VALUES ($1, $2)', [team_id, req.user[0].id], function(err, result) {
									if(err){console.log(err);}
										...
										req.user[0].team_name = req.query.team;
										res.redirect('/account');
										return;
									}
								});
	...
	app.post('/buy' , ( req , res ) => {
		...
		var return_url = "http://localhost:5000/pay-success?team="+req.body.name+"&offer="+req.body.offer;
		var payment = {
				"intent": "authorize",
		"payer": {
...
passport.use('local', new  LocalStrategy({passReqToCallback : true}, (req, username, password, done) => {
	...
	async function loginAttempt() {
		...
		try{
			await client.query('BEGIN')
			var currentAccountsData = await JSON.stringify(client.query('SELECT id, "firstname", "lastname", "email", "password", "birthday", "iam", "pseudo", "city", "weapon" FROM "users" WHERE "email"=$1', [username], function(err, result) {
				...
				if(result.rows[0] == null){
					...
					bcrypt.compare(password, result.rows[0].password, function(err, check) {
						if (err){
							...
							return done(null, [{email: result.rows[0].email, firstname: result.rows[0].firstname, lastname: result.rows[0].lastname, birthday: result.rows[0].birthday, iam: result.rows[0].iam, pseudo: result.rows[0].pseudo, city: result.rows[0].city, weapon: result.rows[0].weapon}]);
						}
						else{
							req.flash('danger', "Incorrect login details.");
							return done(null, false);
