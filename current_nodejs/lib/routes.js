		...
		try{
			if(req.isAuthenticated()){
				...
				await JSON.stringify(client.query('SELECT id FROM "users" WHERE "email"=$1', [req.user[0].email], function(err, result) {
					if(result.rows[0]){
						var dateBirthday = req.body.birthday;
						console.log(dateBirthday);
						client.query('UPDATE users SET firstname=$2, lastname=$3, email=$4, birthday=$5, iam=$6, pseudo=$7, city=$8 WHERE email=$1', [req.user[0].email, req.body.firstname, req.body.lastname, req.body.email, dateBirthday.value, req.body.iam, req.body.pseudo, req.body.city], function(err, result) {
							if(err){console.log(err);}
							else {
							client.query('COMMIT')
								...
								req.user[0].birthday = dateBirthday;
...
passport.use('local', new  LocalStrategy({passReqToCallback : true}, (req, username, password, done) => {
	...
	async function loginAttempt() {
		...
		try{
			await client.query('BEGIN')
			var currentAccountsData = await JSON.stringify(client.query('SELECT id, "firstname", "lastname", "email", "password" FROM "users" WHERE "email"=$1', [username], function(err, result) {
