...
passport.use('local', new  LocalStrategy({passReqToCallback : true}, (req, username, password, done) => {
	...
	async function loginAttempt() {
		...
		try{
			await client.query('BEGIN')
			var currentAccountsData = await JSON.stringify(client.query('SELECT id, "firstname", "lastname", "email", "password", "birthday", "iam", "pseudo", "city" FROM "users" WHERE "email"=$1', [username], function(err, result) {
				...
				if(result.rows[0] == null){
					...
					bcrypt.compare(password, result.rows[0].password, function(err, check) {
						if (err){
							...
							return done(null, [{email: result.rows[0].email, firstname: result.rows[0].firstname, lastname: result.rows[0].lastname, birthday: new Date(result.rows[0].birthday), iam: result.rows[0].iam, pseudo: result.rows[0].pseudo, city: result.rows[0].city}]);
						}
						else{
							req.flash('danger', "Incorrect login details.");
							return done(null, false);
