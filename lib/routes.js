...
module.exports = function (app) {
							...
							if(err){
								...
								client.query('INSERT INTO team_users (team_id, user_id) VALUES ($1, $2)', [teamId, userId], function(err, result) {
									if(err){
										...
										client.query('COMMIT')
										//TODO: Send email to the coach to tell him that a user joined his team
	...
	app.post('/delete-team', async function(req, res) {
		try {
			if(req.isAuthenticated()) {
				if(req.user[0].team.coach_id == req.user[0].id && req.body.email.replace(/ /g,"") == req.user[0].email.replace(/ /g,"")) {
					const client = await pool.connect()
					await client.query('BEGIN')
					await JSON.stringify(client.query('DELETE FROM "team_users" WHERE "team_id"=$1', [req.user[0].team.id], function(err, result) {
						if(err){console.log(err);}
							...
							req.user[0].team.name = null;
							req.user[0].team.offer = null;
							req.flash('success', 'You have deleted your team successfully.');
							res.redirect('/account');
							return;
						}
					}))
