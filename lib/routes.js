...
module.exports = function (app) {
	...
	app.get('/member-area', function (req, res, next) {
		if(req.isAuthenticated()) {
			req.flash('warning', "ready connected.");
	...
	app.get('/account', async function (req, res, next) {
		if(req.isAuthenticated()){
			const client = await pool.connect()
			await client.query('BEGIN')
			await JSON.stringify(client.query('SELECT tu.team_id AS team_id, tu.user_id AS user_id, u.firstname AS firstname, u.lastname AS lastname, u.weapon AS weapon, u.pseudo AS pseudo FROM team_users tu INNER JOIN users u ON (tu.user_id = u.id) WHERE team_id = $1', [req.user[0].team.id], function(err, result) {
				if(result.rows[0]) {
	...
	app.post('/login',	passport.authenticate('local', {
		...
		req.flash('success','Welcome, ' + req.user[0].firstname + ' ! :-)');
		res.redirect('/account');
	});
	...
	app.get('/create-team', function (req, res, next) {
		if(req.isAuthenticated()){
			if(req.user[0].team.name) {
				req.flash('warning', "You already have a team");
				res.redirect('/account');
			}
			else {
	...
	app.get('/upgrade-offer', function (req, res, next) {
		if(req.isAuthenticated()){
			if(req.user[0].id == req.user[0].team.coach_id) {
				if(req.user[0].team.offer != 3) {
					if(req.user[0].team.offer == 1) {
						offerName = 'Bronze';
					} else if(req.user[0].team.offer == 2) {
						offerName = 'Silver';
					} else {
						offerName = 'Gold';
					}

					res.render('upgrade-offer', {title: "Upgrade offer", userData: req.user, messages: {danger: req.flash('danger'), warning: req.flash('warning'), success: req.flash('success')}, offerName: offerName});
				} else {
					req.flash('danger', 'Your team has already a gold subscription');
					res.redirect('/account');
				}
			} else {
	...
	app.get('/add-player', async function (req, res, next) {
		if(req.isAuthenticated()){
			if(req.user[0].id == req.user[0].team.coach_id) {
				const client = await pool.connect()
				await client.query('BEGIN')
				await JSON.stringify(client.query('SELECT COUNT(*) AS count FROM team_users WHERE team_id = $1', [req.user[0].team.id], function(err, result) {
					if(result.rows[0]) {
	...
	app.get('/pay-success', async function(req, res){
		...
		paypal.payment.execute(paymentId, execute_payment_json, async function (error, payment) {
			if(error) {
				...
				req.flash('warning', 'Error during payment execution');
				res.redirect('/account');
			} else {
				const client = await pool.connect()
				await client.query('BEGIN')
				...
				if(insert) {
					await JSON.stringify(client.query('SELECT id FROM "team" WHERE "name"=$1', [payment.transactions[0].custom], function(err, result) {
						if(result.rows[0]){
							...
							client.query('SELECT * FROM team_users WHERE user_id=$1', [req.user[0].id], function(err, result) {
								if(result.rows[0]){
									req.flash('danger', "ready in a team");
									res.redirect('/account');
								}
								else{
									var team_id = uuidv4();
					...
					await JSON.stringify(client.query('UPDATE team SET offer = $1 WHERE id = $2', [teamOffer, req.user[0].team.id], function(err, result) {
						if(err){console.log(err);}
						else {
							client.query('COMMIT')
							
							req.user[0].team.offer = teamOffer;

							req.flash('success', 'Your team has been upgraded !');
							res.redirect('/account');
						}
					}));
				}
	...
	app.post('/pay' , ( req , res ) => {
		...
		if(req.body.offer) {
			...
			teamOffer = req.body.offer;
		} else if(req.body.upgrade) {
			if(req.body.upgrade == 2) {
				teamPrice = "80";
			}

			insert = false;
			teamOffer = req.body.upgrade;
		} else {
			req.flash('danger', 'An unknown error has occured');
			res.redirect('/account');
			return;
		}
