...
module.exports = function (app) {
	...
	app.get('/account', async function (req, res, next) {
		if(req.isAuthenticated()){
			const client = await pool.connect()
			await client.query('BEGIN')
			await JSON.stringify(client.query('SELECT * FROM team_users WHERE team_id = $1', [req.user[0].team.id], function(err, result) {
				if(result.rows[0]) {
	...
	app.get('/add-player', function (req, res, next) {
		// TODO If the team is full or if user is not coach, redirect to account page with an error which say "You are not able to see this page"
		if(req.isAuthenticated()){
			res.render('add-player', {title: "Add a player", userData: req.user, messages: {danger: req.flash('danger'), warning: req.flash('warning'), success: req.flash('success')}});
		}
		else{
			res.redirect('/member-area');
		}
	...
	app.post('/add-player', async function (req, res) {
		try {
			if(req.isAuthenticated()){
				...
				// If the count of team_code is < to team capacity
				client.query('INSERT INTO team_codes (team_id, code) VALUES ($1, $2)', [req.user[0].team.id, code], function(err, result) {
					if(err){console.log(err);}
						...
						var mailOptions = {
							...
							to: req.body.email,
							subject: 'Rejoignez votre nouvelle équipe !',
							text: 'Votre code pour rejoindre la team est: ' + code,
						};
						...
						transporter.sendMail(mailOptions, function(error, info){
							if(error) {
								...
								console.log('Code ' + code + ' sent by email');
								req.flash('warning', 'The code has been sent by email !');
								res.redirect('/account');
							}
						});
					}
	...
	app.post('/join-team', async function (req, res) {
		try {
			if(req.isAuthenticated()){
				const client = await pool.connect()
				await client.query('BEGIN')
				await JSON.stringify(client.query('SELECT * FROM team_codes WHERE code = $1', [req.body.code], function(err, result) {
					if(!result.rows[0]) {
						console.log('Error: code doesn\'t exists');
						req.flash('warning', "Error, this code does not exists");
						res.redirect('/join-team');
					} else if(result.rows[0].is_used == true) {
						console.log('Error: code already used')
						req.flash('warning', "Error, this code has been already used");
						res.redirect('/join-team');
					} else {
						...
						client.query('SELECT * FROM users WHERE email=$1', [req.user[0].email], function(err, result) {
							if(result.rows[0]) {
								...
								req.flash('warning', 'Unknown error has been thrown');
								res.redirect('/join-team');
							}
						});

						client.query('UPDATE team_codes SET is_used = TRUE WHERE code = $1', [req.body.code], function(err, result) {
							if(err){
								console.log(err);
								req.flash('warning', 'Unknown error has been thrown');
								res.redirect('/join-team');
								...
								client.query('INSERT INTO team_users (team_id, user_id) VALUES ($1, $2)', [teamId, userId], function(err, result) {
									if(err){
										...
										client.query('COMMIT')
										// Send email to the coach to tell him that a user joined his team
										// Add the team to user currently login: you have to reconnect for the moment
										console.log(userId + ' joined the team ' + teamId);
										req.flash('warning', 'You have been joined the team');
										res.redirect('/account');
									}
								});
							}
						...
						if(result.rows[0] == null){
							...
							bcrypt.compare(password, result.rows[0].password, function(err, check) {
								if(err){
									...
									return done(null, [{email: result.rows[0].email, firstname: result.rows[0].firstname, lastname: result.rows[0].lastname, birthday: result.rows[0].birthday, iam: result.rows[0].iam, pseudo: result.rows[0].pseudo, city: result.rows[0].city, weapon: result.rows[0].weapon, team: {id: null, coach_id: null, name: null, offer: null}}]);
								}
								else{
									req.flash('danger', "Incorrect login details.");
									return done(null, false);
